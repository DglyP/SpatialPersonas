...
public class ConfigureXcodeSettings : IPreprocessBuildWithReport
{
    ...
    public static void OnPostProcessBuild(BuildTarget buildTarget, string pathToBuiltProject)
    {
        if (buildTarget == BuildTarget.VisionOS)
        {
            ...
            AddModules(buildTarget, pathToBuiltProject);
    ...
    static void AddModules(BuildTarget buildTarget, string pathToBuiltProject)
    {
        // Get the Xcode project path (xcodeproj file)
        string xcodeprojPath = GetXcodeprojPath(pathToBuiltProject);
        var projectPath = xcodeprojPath;
        ...
        // Get the full path to the UnityFramework.modulemap inside the package
        string packagePath = GetCurrentPackageAbsolutePath();
        var moduleFile = Path.Combine(pathToBuiltProject, "UnityFramework/UnityFramework.modulemap");
        var sourceFile = Path.Combine(packagePath, "Editor/FilesToAdd/UnityFramework.modulemap");
        ...
        {
            File.Copy(sourceFile, moduleFile, true);
            project.AddFile(moduleFile, "UnityFramework/UnityFramework.modulemap");
            project.AddBuildProperty(unityFrameworkGuid, "MODULEMAP_FILE", "UnityFramework/UnityFramework.modulemap");
        }
        else
        {
            Debug.LogError("Source file not found: " + sourceFile);
        }
